...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const retrieveSessionIDForSessionsInvitedTo_QueryResults = await db.query(
      'SELECT sessionID FROM Invited_Participant WHERE userID = ?',
      userId
    )
    ...
    if (retrieveSessionIDForSessionsInvitedTo_QueryResults.length > 0) {
      for (
        let i = 0;
        i < retrieveSessionIDForSessionsInvitedTo_QueryResults.length;
        i++
      ) {
        const retriveSessionInfo_QueryResults = await db.query(
          'SELECT * FROM Session WHERE sessionID = ?',
          retrieveSessionIDForSessionsInvitedTo_QueryResults[i].sessionID
        )
        const serverId = retriveSessionInfo_QueryResults[0].serverID
        const hostUserId = retriveSessionInfo_QueryResults[0].hostUserID
        const retrieveServerInfo_QueryResults = await db.query(
          'SELECT hostIP, hostPort FROM Server WHERE serverID = ?',
          serverId
        )
        const retirveHostUserName_QueryResults = await db.query(
    ...
    const retrieveSessionsWhereUserIsHost_QueryResults = await db.query(
      'SELECT * FROM Session WHERE hostUserID = ?',
      userId
    )
    if (retrieveSessionsWhereUserIsHost_QueryResults.length > 0) {
      for (
        let i = 0;
        i < retrieveSessionsWhereUserIsHost_QueryResults.length;
        i++
      ) {
        const serverId =
          retrieveSessionsWhereUserIsHost_QueryResults[i].serverID
        const retrieveServerInfo_QueryResults = await db.query(
          'SELECT hostIP, hostPort FROM Server WHERE serverID = ?',
          serverId
        )
        sessionsArray.push({
          ...
          sessionName:
            retrieveSessionsWhereUserIsHost_QueryResults[i].sessionName,
          mapName: retrieveSessionsWhereUserIsHost_QueryResults[i].mapName,
          maxParticipants:
            retrieveSessionsWhereUserIsHost_QueryResults[i].maxParticipants,
          participantCount:
            retrieveSessionsWhereUserIsHost_QueryResults[i].participantCount,
  ...
  connection.query(
    ...
    function (error, results, fields) {
      if (error) {
        ...
        connection.query(
          'SELECT serverID FROM Server WHERE isAllocated = ?',
          false,
          function (error, results, fields) {
            if (error) {
              res.status(500)
              ...
              connection.query(
                'SELECT * FROM Session WHERE sessionName = ? AND hostUserID = ?',
                [SessionDetails.sessionName, hostId],
                function (error, results, fields) {
                  if (error) {
                    res.status(500)
                    // PRINT OUT THE SPECIFIC ERROR
                    ...
                    res.send({
                      message:
                        'Session with the same name already exists for the user'
                    })
                  } else {
                    connection.query(
                      'INSERT INTO Session (sessionName, mapName, maxParticipants, serverID, hostUserID, participantCount) VALUES (?, ?, ?, ?, ?, ?)',
                      [
                        SessionDetails.sessionName,
                        SessionDetails.mapName,
                        SessionDetails.maxParticipants,
                        serverId,
                        hostId,
                        0
                      ],
                      function (error, results, fields) {
                        if (error) {
                          ...
                          connection.query(
                            'UPDATE Server SET isAllocated = ? WHERE serverID = ?',
                            [true, serverId],
                            function (error, results, fields) {
                              if (error) {
                                res.status(500)
...
router.delete('/:sessionId', function (req, res) {
  ...
  connection.query(
    'SELECT serverID FROM Session WHERE sessionID = ?',
    sessionId,
    function (error, results, fields) {
      if (error) {
        res.status(500)
        ...
        const serverId = results[0].serverID
        connection.query(
          'DELETE FROM Invited_Participant WHERE sessionID = ?',
          sessionId,
          function (error, results, fields) {
            if (error) {
              res.status(500)
