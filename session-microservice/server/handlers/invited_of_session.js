...
router.post('/', async (req, res) => {
  ...
  try {
      for (let i = 0; i < arrayOfInvited.length; i++) {
        ...
        const retrieveUserID_QueryResults = await db.query('SELECT userID FROM User WHERE username = ? OR userEmail = ?', [usernameOrEmail, usernameOrEmail]);
        ...
        const retrivevAllInvitedToSession_QueryResults = await db.query('SELECT * FROM Invited_Participant WHERE userID = ? AND sessionID = ?', [userId, sessionId]);
        // Check if user is not already invited
        if (retrivevAllInvitedToSession_QueryResults.length > 0) {
            continue;
        } 
        await db.query('INSERT INTO Invited_Participant (userID, sessionID) VALUES (?, ?)', [userId, sessionId]);
      };
